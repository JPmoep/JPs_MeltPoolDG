...
namespace MeltPoolDG
{
  ...
  namespace UtilityFunctions
  {
    ...
    namespace DistanceFunctions
    {
      ...
      template <int dim>
      inline double infinite_line(const Point<dim>& p, const Point<dim>& fix_p1, const Point<dim>& fix_p2)
      {
        if (dim == 3)
      ...
      /**
       *  This function defines the signed distance function of a rectangular manifold. The lower left corner
       *  of the rectangle (lowest values for the x,y,z coordinates among the corner points) and the upper
       *  left corner (highest values of the x,y,z coordinates) must be provided as input. Inside the rectangle
       *  a positive and outside the rectangle a negative value for the distance function is considered.
       */
      template <int dim>
      inline double
      rectangular_manifold(const Point<dim> &p,const Point<dim> &lower_left_corner, const Point<dim> &upper_right_corner)
      {
        ...
        if (dim == 3)
        {
          AssertThrow(false, ExcMessage("Not implemented yet. Rectangular manifold: dim must be 2."));
        }
        else if (dim == 2)
        {
          Point<dim> center;
