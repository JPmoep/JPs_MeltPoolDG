...
using namespace dealii;
  ...
  VectorTools
  {
    ...
    void
    convert_fe_sytem_vector_to_block_vector(const LinearAlgebra::distributed::Vector<Number>& in, const DoFHandler<dim, spacedim> & dof_handler_fe_system, LinearAlgebra::distributed::BlockVector<Number>& out, const DoFHandler<dim, spacedim> & dof_handler)
    {
    ...
    template<int dim, int spacedim, typename Number>
    void
      convert_block_vector_to_fe_sytem_vector(const LinearAlgebra::distributed::BlockVector<Number>& in, const DoFHandler<dim, spacedim> & dof_handler, LinearAlgebra::distributed::Vector<Number>& out, const DoFHandler<dim, spacedim> & dof_handler_fe_system)
      {
        in.update_ghost_values();
    ...
    template<typename... T>
    void update_ghost_values(const T&... args)
    {
        ((args.update_ghost_values()), ...);
    }
    
    template<typename... T>
    void zero_out_ghosts(const T&... args) 
    {
