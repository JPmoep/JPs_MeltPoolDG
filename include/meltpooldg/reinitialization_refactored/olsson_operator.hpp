...
class OlssonOperator : public ReinitializationOperatorBase<dim,degree,number>
{
  ...
  public:
      ...
      print_me()
      {
        std::cout << "hello from child class olsson_operator" << std::endl;
        std::cout << "hello from child class olsson_operator : n_dofs" << this->dof_handler->n_dofs() << std::endl;
      }

      void
      initialize_matrixfree()
      {
      }

      void
      initialize_matrixbased()
      {
        //fe_values( fe,
                   //q_gauss,
      ...
      //void
      //assemble_matrixbased( const VectorType& src, 
                            //SparseMatrixType & matrix, 
                            //BlockVectorType & rhs,
                            //const unsigned int n_q_points,
                            //const unsigned int dofs_per_cell,
                            //SmartPointer<const DoFHandler<dim>> dof_handler,
                            //SmartPointer<const AffineConstraints<number>> constraints ) const
      //{
        ...
        //{
          ...
          //fe_values.get_function_values(     solution_in, psi_at_q );     // compute values of old solution at tau_n
          //fe_values.get_function_gradients(  solution_in, grad_psi_at_q ); // compute gradients of old solution at tau_n
        ...
        matrix_free.template cell_loop<VectorType, VectorType>( [&] 
          (const auto&, auto& dst, const auto& src, auto cell_range) {
            for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
            {
              ...
              normal_vector.read_dof_values(this->normal_vector_field.normal_vector_field);
    ...
    create_rhs(VectorType & dst,
               const VectorType & src) const
    {
      std::cout << "d_tau: " << this->d_tau << std::endl;
