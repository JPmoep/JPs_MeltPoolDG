...
namespace SlottedDisc
{
  ...
  class ExactSolution : public Function<dim> 
  {
                         ...
                         const unsigned int component = 0) const 
    {
      ...
      // return the smallest distance
      std::cout << "minimal distance d_line_min:" << d_min << "  d_manifold:" << d_manifold << "\n";
      return UtilityFunctions::CharacteristicFunctions::tanh_characteristic_function(d_min, eps_interface);
    }
    private:
    ...
    public:
      ...
      Tensor<1, dim> value(const Point<dim> & p) const 
      {
        ...
        if constexpr (dim==2)
        {
          ...
          value_[0] = 4*y; // numbers::PI/314 * (50 - y);      //4*y;
          value_[1] = -4*x; // numbers::PI/314 * (x - 50);      //-4*x;
        }
        else
 ...
 /* template <int dim>
  ...
  {
  public:
     SimulationSlottedDisc(std::string parameter_file,
    ...
    {   
      /*  this->field_conditions.initial_field =        UtilityFunctions::CharacteristicFunctions::sgn(
                                                          std::make_shared<InitializePhi<dim>>()); 
        this->field_conditions.exact_solution_field = UtilityFunctions::CharacteristicFunctions::tanh_characteristic_function(
                                                          std::make_shared<InitializePhi<dim>>(), 0.01);
      */
       // TODO: epsilon als parameter?!
