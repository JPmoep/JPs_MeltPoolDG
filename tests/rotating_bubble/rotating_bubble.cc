...
namespace MeltPoolDG
{
  namespace Test
  {
    namespace RotatingBubble
    {
      ...
      {
      public:
        Simulation(std::string parameter_file, const MPI_Comm mpi_communicator)
...
main(int argc, char *argv[])
{
  ...
  std::shared_ptr<SimulationBase<2>> test =
    std::make_shared<Simulation<2>>(input_file, mpi_communicator);
  test->create();
